...
export default function AdminLogin() {
  ...
  return <main className="login"><form className="login-card" onSubmit={submit}><div className="brand">Offer<span>Flow</span></div><h1>Seller login</h1><p>Manage products and publish them to your store.</p>{error && <div className="notice">{error}</div>}<label className="label" htmlFor="password">Admin password</label><input className="field" id="password" name="password" type="password" required/><br/><br/><button className="primary full">Sign in</button></form></main>;
}
